...
- [ ] 起動時の通信量を削減する（低速回線・スマホ向け）
  - 対応前: 2026-09-11のビルド集計では配信ファイル約89.5MB、うちキャラクター素材約87.9MB。PWA precacheが全キャラ画像を初回起動時に先読みしていた。実通信量は圧縮・キャッシュで変わり、毎回この容量を取得するわけではない
  - [x] 優先1: 全キャラ画像を起動時のprecacheから外し、ゲーム本体など起動に必要なファイルだけを先読みする
  - [x] 優先2: 使用するキャラの必要な画像を取得・キャッシュする。設定の「オフライン用データ」から全キャラを明示的に保存でき、中止・再試行では保存済みを再利用。未取得画像とブラウザによる保存消去についても案内する
  - [ ] 優先3: 配信用PNGのWebP化と解像度を検討。画質・透過・アニメーションを比較して容量を削減する（制作原本は維持）
  - 検証: キャッシュなしの初回起動・再起動・更新時・対戦開始時の通信量を分けて測定し、低速回線でも必要素材が先に届くこと、オフライン起動・取得済みキャラの表示が壊れないことを確認する
- [x] 描画解像度をスマホに合わせる
