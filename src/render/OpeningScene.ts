 ...
 * 効果音は AudioContext が動いているときだけ鳴る（初回の起動は操作の前なので無音）。
 */

/** オープニングの進み具合。e2e で確かめる。 */
export type OpeningPhase = "rise" | "swap" | "chain" | "reveal" | "done";
...
export class OpeningScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // パネル。最初の S の角のパネルはひとつ左にずらして置く（swap で戻す）。
    // 盤面の次の行と同じく暗い柄で始め、せり上がりが止まったら灯す
    ...
    for (const cell of logo.cells) {
      ...
      this.tweens.add({
        targets: img,
        y: cellY(cell.row),
        delay: cell.col * stagger,
        duration: 460,
        ease: "Back.Out",
        easeParams: [1.0],
        onComplete: () => img.setTexture(`panel-${cell.kind}`),
      });
    }

    // 2. swap。カーソルが現れ、ずれたパネルを元の位置へ入れ替える
    ...
    // 3. chain。文字ごとに 点滅 → 1 枚ずつ消える。次の文字は消え始めてすぐに点滅を始めるので、連鎖が走って見える
    this.at(T.chain, () => {
      this.phase = "chain";
    });
    ...
    // 4. reveal。閃光と揺れのあと、文字の題字が中央に現れ、メニューの位置へ上がる
    ...
    // 途中で飛ばす。キー・タップ・ゲームパッドのどれでも
    this.input.keyboard?.on("keydown", () => this.finish());
    this.input.on("pointerdown", () => this.finish());
    this.input.gamepad?.on("down", () => this.finish());
