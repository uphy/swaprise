...
import { SCORE_RULES, validSubmission, type Submission, type ScoreMode, type RankedScore } from "./model";
...
export function enqueueScore(score: Omit<Submission, "name" | "rules">): void {
  ...
  const value = { ...score, name: displayName(playerName()), rules: SCORE_RULES };
...
export async function ranking(mode: ScoreMode, signal: AbortSignal): Promise<RankedScore[]> {
  const response = await fetch(`/api/scores?mode=${mode}&rules=${SCORE_RULES}`, { signal: AbortSignal.any([signal, AbortSignal.timeout(8000)]) });
