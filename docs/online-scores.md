...
- 投稿はプレイ単位の UUID、mode、score、maxChain、name、seed、frames、rules。エンドレスは `scores-v1`、時間切れ後の連鎖を含むタイムアタックは `scores-ta-v2`。`src/scores/model.ts` の `scoreRules(mode)` で分離し、異なるルールのランキングを混ぜない（対戦の `GAME_VERSION` とは独立）。旧タイムアタック記録はDBに保持するが現在の一覧には出さず、旧ルールの未送信分は新ルールとして再送しない。
