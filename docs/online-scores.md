...
`migrations/0001_scores.sql` は追記型の初回 migration。ランキングとレート制限に専用インデックスを使い、上位取得で全件走査しない。`0002_player.sql` が `player` 列と `(rules, mode, player, ...)` のインデックスを足し、既存行の `player` にはプレイ id を入れる。API と対戦の有効化は現状ともに `ONLINE_ENABLED` で制御する。
